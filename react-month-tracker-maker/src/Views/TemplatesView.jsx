...
function TemplatesView() {
  ...
  return (
    <Box sx={{ backgroundColor: '#f0f0f0' }}>
      <Grid container sx={{ margin: 1, justifyContent: 'center', alignItems: 'center' }}>
        {templatesData.map((item, index) => (
          <Grid item xs={12} sm={6} md={3} key={index} sx={{ margin: 1, minWidth: { xs: '100%', sm: '400px' } }}>
              <Calendar 
              ...
              emojis={item.emojis} 
              />
            <Stack direction={"row"} spacing={1} margin={1}  sx={{ width: { xs: '100%', sm: '380px' }}}>
